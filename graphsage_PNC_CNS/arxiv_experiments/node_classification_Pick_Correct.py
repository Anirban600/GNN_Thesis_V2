...
def evaluate(model, graph, dataloader):
    ...
    return MF.accuracy(torch.cat(y_hats), torch.cat(ys), task='multiclass', num_classes=40)
...
def layerwise_infer(device, graph, nid, model, batch_size):
    ...
    with torch.no_grad():
        ...
        return MF.accuracy(pred, label, task='multiclass', num_classes=40)
...
#                               prefetch_labels=['label'])
    ...
    def give_train_data():
        sample_idx = np.random.choice(train_idx.cpu(), size=len(train_idx)//4, replace=False, p=probs)
        sample_idx = torch.tensor(sample_idx, device=device)
        
        train_dataloader = DataLoader(g, sample_idx, sampler_default, device=device,
