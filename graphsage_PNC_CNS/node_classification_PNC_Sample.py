...
from pick import LabelBalancedSampler
...
def run(args, device, g, dataset):
    ...
    lbs = LabelBalancedSampler(A, g.ndata["label"])
    probs = lbs.all_probabilities().to(device)
    g.edata["prob"] = probs[g.edges()[0]]
    
    
    sampler_pick = NeighborSampler([10, 10, 10],  # fanout for [layer-0, layer-1, layer-2]
                          prob="prob",
                          prefetch_node_feats=['feat'],
                          prefetch_labels=['label'])

    sampler_original = NeighborSampler([10, 10, 10],  # fanout for [layer-0, layer-1, layer-2]
                          prefetch_node_feats=['feat'],
                          prefetch_labels=['label'])
    
    train_dataloader = DataLoader(g, train_idx, sampler_pick, device=device,
                                  batch_size=1024, shuffle=True,
                                  drop_last=False, num_workers=0,
                                  use_uva=use_uva)

    val_dataloader = DataLoader(g, val_idx, sampler_original, device=device,
                                batch_size=1024, shuffle=True,
                                drop_last=False, num_workers=0,
                                use_uva=use_uva)
