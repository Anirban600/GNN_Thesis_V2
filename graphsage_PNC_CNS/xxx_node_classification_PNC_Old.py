...
def train(args, device, g, dataset, model):
    ...
    sg = dgl.dataloading.NeighborSampler([1], prob='prob')
    
    loader = dgl.dataloading.DataLoader(
        g, [0, 1], sg,
        batch_size=100, shuffle=True, drop_last=False, num_workers=4)
    ...
    for a, b, c in loader:
        ...
        print("="*50)
        print(c[0].edges())
        print(c[0].srcdata[dgl.NID])
        print(c[0].dstdata[dgl.NID])
        print("="*50)
    ...
    for epoch in range(10):
        ...
        print(f"Epoch {epoch:05d} | Loss {total_loss / (it+1):.4f} | Accuracy {acc.item():.4f} | Time : {int(time.time() - t)}s")
